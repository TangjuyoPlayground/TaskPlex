...
import { usePyValidator } from '../hooks/usePyValidator';

export const PyValidatorScreen: React.FC = () => {
  const { t } = useTranslation();
  const [python, setPython] = useState('');
  const { mutate, data, isPending, error, reset } = usePyValidator();
    ...
    <div className="p-8 max-w-6xl mx-auto">
      <div className="flex flex-col items-center text-center gap-3 mb-8">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">{t('pyValidator.title')}</h1>
        </div>
        <p className="text-gray-600 dark:text-gray-400 max-w-3xl">{t('pyValidator.description')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col">
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
            {t('pyValidator.inputLabel')}
          </label>
          <textarea
            value={python}
            onChange={(e) => setPython(e.target.value)}
            placeholder={t('pyValidator.inputPlaceholder')}
          ...
          />
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">{t('pyValidator.inputHint')}</p>

          <div className="mt-4 flex items-center gap-3">
            <button
              onClick={handleValidate}
              disabled={isPending || !python.trim()}
            ...
            >
              ...
              {isPending ? t('pyValidator.validating') : t('pyValidator.validateButton')}
            </button>
            <button
              onClick={handleReset}
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col min-h-[320px]">
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('pyValidator.results')}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">{t('pyValidator.resultsHint')}</p>
            </div>
            {data && (
              <span
              ...
              >
                ...
                {isValid ? t('pyValidator.valid') : t('pyValidator.invalid')}
              </span>
            )}
          </div>
          ...
          <div className="flex-1 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg p-4 overflow-auto">
            ...
            {data && isValid && (
              <div className="flex items-center gap-2 text-green-700 dark:text-green-300">
                ...
                <span>{t('pyValidator.noErrors')}</span>
              </div>
            )}
