...
export const Base64EncodeScreen: React.FC = () => {
  const { t } = useTranslation();
  const [input, setInput] = useState('');
  const [copied, setCopied] = useState(false);

  const mutation = useBase64('encode');
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t('base64Encode.title')}</h1>
          <p className="text-sm text-gray-600 dark:text-gray-400">{t('base64Encode.subtitle')}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-2 h-full flex flex-col">
          <label className="text-sm font-medium text-gray-700 dark:text-gray-200">
            {t('base64Encode.inputLabel')}
          </label>
          <p className="text-xs text-gray-500 dark:text-gray-400">
            {t('base64Encode.inputHint')}
          </p>
          <div className="flex-1">
            <textarea
              ...
              placeholder={t('base64Encode.placeholder')}
            />
          </div>
          ...
          <ProcessButton
            ...
            disabled={!canSubmit}
            loading={loading}
            labelKey="base64Encode.encode"
            loadingLabelKey="base64Encode.processing"
            color="blue"
          />
        ...
        <div className="space-y-3 h-full flex flex-col">
          <div className="flex items-center justify-between">
            <div>
              <label className="text-sm font-medium text-gray-700 dark:text-gray-200">
                {t('base64Encode.output')}
              </label>
              <p className="text-xs text-gray-500">{t('base64Encode.outputHint')}</p>
            </div>
            <button
              type="button"
              onClick={handleCopy}
            ...
            >
              ...
              {copied ? t('base64Encode.copied') : t('base64Encode.copy')}
            </button>
          </div>
          <div className="flex-1">
            <textarea
              ...
              placeholder={t('base64Encode.outputPlaceholder')}
            />
          </div>
        </div>
      </div>
