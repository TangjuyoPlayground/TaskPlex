...
import { FileJson, Upload, Download, FileText } from 'lucide-react';
import { useTranslation } from 'react-i18next';
import { ApiService } from '../services/api';
import { useCSVToJSON } from '../hooks/useCSVConverter';
import { useDownload } from '../hooks/useDownload';

export const CSVToJSONScreen: React.FC = () => {
  const { t } = useTranslation();
  const [file, setFile] = useState<File | null>(null);
  
  const { mutate, isPending: loading, data: result, error, reset } = useCSVToJSON();
  ...
  const handleDownload = useCallback(() => {
    if (result?.download_url) {
      ...
      const filename = result.download_url.split('/').pop() || 'converted.json';
    ...
    <div className="p-8 max-w-4xl mx-auto">
      <div className="text-center mb-10">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2 flex items-center justify-center gap-3">
          <FileJson className="text-teal-600 dark:text-teal-400" size={32} />
          {t('modules.data.csvToJson.title')}
        </h1>
        <p className="text-gray-600 dark:text-gray-400">{t('modules.data.csvToJson.description')}</p>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-8">
        {!file ? (
          <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-12 text-center hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors relative group cursor-pointer">
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{t('modules.data.csvToJson.selectFile')}</h3>
            <p className="text-gray-500 dark:text-gray-400">{t('modules.data.csvToJson.dropFile')}</p>
          </div>
        ) : (
          <div className="space-y-8">
            <div className="flex items-center gap-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-xl border border-gray-200 dark:border-gray-600">
              ...
              >
                {t('modules.data.csvToJson.change')}
              </button>
            </div>
            ...
            {!result || !result.success ? (
              ...
              >
                {loading ? t('modules.data.csvToJson.converting') : t('modules.data.csvToJson.convertBtn')}
              </button>
            ) : (
              <div className="bg-teal-50 dark:bg-teal-900/30 border border-teal-100 dark:border-teal-800 rounded-xl p-6 text-center animate-in fade-in slide-in-from-bottom-4">
                ...
                {result.rows_count > 0 && (
                  <p className="text-teal-700 dark:text-teal-300 mb-6">
                    {t('modules.data.csvToJson.successMessage', { count: result.rows_count })}
                  </p>
                )}
                {result.download_url && (
                  <button
                  ...
                  >
                    ...
                    {t('modules.data.csvToJson.downloadResult')}
                  </button>
                )}
              </div>
            )}
