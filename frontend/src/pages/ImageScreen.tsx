      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
            <label className="block text-sm font-medium text-gray-700 mb-3">{t('image.operation')}</label>
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
            <label className="block text-sm font-medium text-gray-700 mb-3">{t('image.imageFile')}</label>
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
            <label className="block text-sm font-medium text-gray-700 mb-3">{t('image.quality')}</label>
          ...
          {operation === 'convert' && (
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
              <label className="block text-sm font-medium text-gray-700 mb-3">{t('image.outputFormat')}</label>
              <select
                value={format}
          ...
          >
            {loading ? t('image.processing') : operation === 'compress' ? t('image.compressImage') : t('image.convertImage')}
          </button>
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider">{t('image.original')}</h3>
              <div className="bg-gray-100 rounded-xl border border-gray-200 overflow-hidden aspect-square flex items-center justify-center relative">
                {previewUrl ? (
                  <img src={previewUrl} alt={t('image.original')} className="max-w-full max-h-full object-contain" />
                ) : (
            ...
            <div className="space-y-2">
              <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider flex items-center justify-between">
                {t('image.result')}
                {result && result.success && <span className="text-green-600 font-bold">{t('common.success')}!</span>}
              </h3>
              <div className="bg-gray-100 rounded-xl border border-gray-200 overflow-hidden aspect-square flex items-center justify-center relative">
                {result?.download_url ? (
                  <img 
                ...
                <div>
                  ...
                  {result.compression_ratio && (
                    <p className="text-green-700 text-sm mt-1">
                      {t('image.reducedBy')} <span className="font-bold">{result.compression_ratio.toFixed(1)}%</span>
                    </p>
                  )}
                </div>
