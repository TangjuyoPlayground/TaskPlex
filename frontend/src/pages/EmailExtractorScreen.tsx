...
import { useEmailExtractor } from '../hooks/useTextExtractor';
import { ProcessButton } from '../components/ui/ProcessButton';
import { ErrorAlert } from '../components/ui/ErrorAlert';

export const EmailExtractorScreen: React.FC = () => {
  const { t } = useTranslation();
  const [text, setText] = useState('');
  const [copied, setCopied] = useState(false);
  const { mutate, data: result, isPending: loading, error, reset } = useEmailExtractor();
    ...
    <div className="p-8 max-w-6xl mx-auto min-h-screen">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2 flex items-center justify-center gap-3">
          ...
          {t('emailExtractor.title')}
        </h1>
        <p className="text-gray-600 dark:text-gray-400">{t('emailExtractor.description')}</p>
      </div>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
            ...
            <textarea
              ...
              placeholder={t('emailExtractor.inputPlaceholder')}
            ...
            />
            <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">{t('emailExtractor.inputHint')}</p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <ProcessButton
              ...
              loading={loading}
              labelKey="emailExtractor.extract"
              loadingLabelKey="emailExtractor.extracting"
              color="blue"
            />
            <button
              onClick={handleReset}
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between mb-4">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                {t('emailExtractor.results')} {result?.count !== undefined && `(${result.count})`}
              </label>
              ...
              {result?.emails && result.emails.length > 0 ? (
                <div className="space-y-2">
                  {result.emails.map((email, index) => (
                    <div
                      key={index}
                      className="p-2 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 rounded border border-blue-200 dark:border-blue-800 font-mono text-sm"
                    >
                      {email}
                    </div>
                  ))}
                ...
                <p className="text-gray-400 dark:text-gray-500 text-sm">{t('emailExtractor.placeholder')}</p>
              )}
            </div>
          </div>
        </div>
