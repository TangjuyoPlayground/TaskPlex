...
def convert_pdf_to_word(input_path: Path, output_path: Path) -> PDFProcessingResponse:
    """
    Convert a PDF to a DOCX (text-only extraction).
    """
    try:
...
def _extract_docx_text(docx_path: Path) -> str:
    ...
    # Extract text from tables as well (row by row)
...
def convert_word_to_pdf(input_path: Path, output_path: Path) -> PDFProcessingResponse:
    """
    Convert a DOCX to PDF (text-only rendering).
    """
    try:
        original_size = get_file_size(input_path)

        content = _extract_docx_text(input_path)
        if not content:
            content = " "  # keep a non-empty PDF even if DOCX has no text

        # Split content into chunks to avoid empty renders on long text
        max_chars = 1800
        ...
        for chunk in chunks:
            ...
            page.insert_textbox(rect, chunk, fontsize=12, fontname="helv", align=0)
